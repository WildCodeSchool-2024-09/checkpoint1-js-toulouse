...
function findSeniors(persons) {
  const webSeniors = persons.filter((elem) => {
    return (elem.experience >= 5) && elem.job === 'web dev';
  });

  const dataSeniors = persons.filter((elem) => {
    return (elem.experience >= 5) && elem.job === 'data analyst';
  });
