...
function findSeniors(persons) {
  const seniors = persons.filter(person => person.job == 'web dev' && person.experience >= 5);
  const analysts = persons.filter(person => person.job == 'data analyst' && person.experience >= 5);
  return [
    seniors,
    analysts
  ]
